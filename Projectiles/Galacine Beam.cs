...
{
	public class GalacineBeam : ModProjectile
	{
		...
		public override void AI()
		{
			...
			if (projectile.type != ModContent.ProjectileType<GalacineBeam>() || !hostPrism.active || hostPrism.type != ModContent.ProjectileType<GalacineHoldout>())
			{
				projectile.Kill();
				return;
			}
			...
			float chargeRatio = MathHelper.Clamp(hostPrism.ai[0] / GalacineHoldout.MaxCharge, 0f, 1f);
			...
			projectile.friendly = hostPrism.ai[0] > GalacineHoldout.DamageStart;

			// This offset is used to make each individual beam orient differently based on its Beam ID.
			float beamIdOffset = BeamID - GalacineHoldout.NumBeams / 2f + 0.5f;
			float beamSpread;
			float spinRate;
			...
			float deviationAngle = (hostPrism.ai[0] + beamIdOffset * spinRate) / (spinRate * GalacineHoldout.NumBeams) * MathHelper.TwoPi;
		...
		private Color GetOuterBeamColor()
		{
			...
			float hue = (BeamID / GalacineHoldout.NumBeams) % BeamHueVariance + BeamColorHue;
